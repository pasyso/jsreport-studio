...
module.exports = (reporter, definition) => {
  ...
  reporter.on('express-configure', () => {
    if (reporter.options.mode !== 'jsreport-development') {
      ...
      fs.writeFileSync(path.join(__dirname, '../src/extensions_dev.js'), reporter.extensionsManager.extensions.map((e) => {
        try {
          ...
          const extensionPath = definition.options.extensionsInDevMode && !definition.options.extensionsInDevMode.includes(e.name)
          ? path.join(e.directory, '/studio/main.js') : path.join(e.directory, '/studio/main_dev.js')
    ...
    if (reporter.options.mode === 'jsreport-development') {
      ...
      const webpackConfig = require('../webpack/dev.config')(reporter.extensionsManager.extensions)
