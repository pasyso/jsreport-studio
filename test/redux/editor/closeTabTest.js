...
describeAsyncStore('editor.actions.closeTab', ({ store, api, history }) => {
  itAsync('should remove tab from collection and unload', async () => {
    ...
    should(store.getState().editor.activeTab).not.be.ok()
  ...
  itAsync('should activate the next tab if the current is active', async () => {
    store.update({
      editor: { tabs: [{ key: '1' }, { key: '2' }], activeTab: '1' }
    })

    await store.dispatch(actions.closeTab('1'))

    store.getState().editor.activeTab.should.be.eql('2')
  })

  itAsync('should activate the closest tab if the current is active', async () => {
    store.update({
      editor: { tabs: [{ key: '1' }, { key: '2' }], activeTab: '2' }
    })
  ...
  itAsync('should update lastActiveTemplateKey with new activeTab', async () => {
    store.update({
      editor: { tabs: [{ key: '1' }, { key: '2', _id: '2', entitySet: 'templates' }], activeTab: '1' }
    })
